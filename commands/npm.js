...
class NPMCommand extends Command {
	...
	async exec(message, { pkg }) {
		...
		if (res.status === 404) {
			return message.channel.send("I couldn't find the requested information. Maybe look for something that actually exists the next time!");
		}
		const body = await res.json();
		if (body.time.unpublished) {
			return message.channel.send('whoever was the Commander of this package decided to unpublish it, what a fool.');
		}
